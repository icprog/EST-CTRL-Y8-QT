...
CMotionCeil12a::CMotionCeil12a(QWidget *parent) :
    CMotionBase12(parent)
{

    setObjectName("CMotionCeil12a");
...
void CMotionCeil12a::myMotionInit()
{
...
void CMotionCeil12a::OnViewDirChange(TypeViewDir sAngle)
{
    switch(sAngle)
    {
    case VDirNORM:
...
void CMotionCeil12a::OnMotionFresh()
{
    ...
    for(int mIdx = 0 ;mIdx < RAIL8_MOTION_MAX ;mIdx++)
    {
        m_iMotionDataCur[mIdx] = g_sysDb->CurrentData[mIdx];
    ...
    if(m_iMotionFresh[MOTOR_FID])
        m_iMotionFresh[MOTOR_ANG] = true;
    ...
    if(m_iMotionFresh[MOTOR_FID])
    {
        ...
        if(m_iMotionDataCur[MOTOR_FID] < FID_DATA_MIN)
            m_iMotionDataCur[MOTOR_FID] = FID_DATA_MIN;

        if(m_iMotionDataCur[MOTOR_FID] > FID_DATA_MAX)
            m_iMotionDataCur[MOTOR_FID] = FID_DATA_MAX;

        if(g_sysDb->g_startAngle == VDirNORM )
        {
            int FID_PIX_MIN   =  SYS_WID(0);
            int FID_PIX_MAX   =  SYS_WID(100);
            fRatio =(float) (FID_PIX_MIN - FID_PIX_MAX )/(FID_DATA_MAX - FID_DATA_MIN);
            tempPositionY = (m_iMotionDataCur[MOTOR_FID] - FID_DATA_MAX) * fRatio + FID_PIX_MIN;
    ...
    //2//////////////////////////////////////立柱探测器升降
    if(m_iMotionFresh[MOTOR_DTH])//
    {
        tempPositionY = rectDet.top();
        tempPositionX = rectDet.left();

        if(m_iMotionDataCur[MOTOR_DTH] < DTH_DATA_MIN)
            m_iMotionDataCur[MOTOR_DTH] = DTH_DATA_MIN;

        if(m_iMotionDataCur[MOTOR_DTH] > DTH_DATA_MAX)
            m_iMotionDataCur[MOTOR_DTH] = DTH_DATA_MAX;
        ...
        if(g_sysDb->g_startAngle == VDirNORM )
        {
            ...
            tempPositionY = (m_iMotionDataCur[MOTOR_DTH] - DTH_DATA_MAX) * fRatio + DTH_PIX_MIN;
        }else
        {
    ...
    //3//////////////////////////////////////球管转动
    if(m_iMotionFresh[MOTOR_ANG])
    {

        if( m_iMotionDataCur[MOTOR_ANG] < ANG_DATA_MIN)
            m_iMotionDataCur[MOTOR_ANG] = ANG_DATA_MIN;

        if( m_iMotionDataCur[MOTOR_ANG] > ANG_DATA_MAX)
            m_iMotionDataCur[MOTOR_ANG] = ANG_DATA_MAX;

        viewTube->rotate( m_iMotionDataCur[MOTOR_ANG] -  m_iMotionDataPre[MOTOR_ANG]);
    }

    //4//////////////////////////////////////立柱移动控制
    if(m_iMotionFresh[MOTOR_SID])
    {
        tempPositionX = rectStand.left();
        tempPositionY = rectStand.top();

        if(m_iMotionDataCur[MOTOR_SID]< SID_DATA_MIN)
            m_iMotionDataCur[MOTOR_SID] = SID_DATA_MIN;
        if(m_iMotionDataCur[MOTOR_SID]> SID_DATA_MAX)
            m_iMotionDataCur[MOTOR_SID] = SID_DATA_MAX;
        ...
        if(g_sysDb->g_startAngle == VDirNORM )
        {
            ...
            tempPositionX = (m_iMotionDataCur[MOTOR_SID] - SID_DATA_MIN) * fRatio +SID_PIX_MIN;
        }else
        {
    ...
    //5//////////////////////////////////////床面升降
    if(m_iMotionFresh[MOTOR_BEH])
    {
        tempPositionX = rectTable.left();
        tempPositionY = rectTable.top();

        if(m_iMotionDataCur[MOTOR_BEH] < TABLE_DATA_MIN)
            m_iMotionDataCur[MOTOR_BEH] = TABLE_DATA_MIN;

        if(m_iMotionDataCur[MOTOR_BEH] > TABLE_DATA_MAX)
            m_iMotionDataCur[MOTOR_BEH] = TABLE_DATA_MAX;
        ...
        if(g_sysDb->g_startAngle == VDirNORM )
        {
            ...
            tempPositionY = (m_iMotionDataCur[MOTOR_BEH] - TABLE_DATA_MAX) * fRatio +TABLE_PIX_MIN;
    ...
    //6//////////////////////////////////////床下探测器左右
    if(m_iMotionFresh[MOTOR_DTP])//
    {
        tempPositionX = rectTableDet.left();
        tempPositionY = rectTableDet.top();

        if(m_iMotionDataCur[MOTOR_DTP] < TABLE_DET_DATA_MIN)
            m_iMotionDataCur[MOTOR_DTP] = TABLE_DET_DATA_MIN;

        if(m_iMotionDataCur[MOTOR_DTP] > TABLE_DET_DATA_MAX)
            m_iMotionDataCur[MOTOR_DTP] = TABLE_DET_DATA_MAX;
        ...
        if(g_sysDb->g_startAngle == VDirNORM )
        {
            ...
            tempPositionX = (m_iMotionDataCur[MOTOR_DTP] - TABLE_DET_DATA_MIN) * fRatio +TABLE_DET_PIX_MIN;
        }else
        {
